...
class cell(object):
	def __init__(self, kxs, kys, kzs, func, i, j, k):
		...
		self.values = (self.func(kxs[i], kys[j], kyz[k]), self.func(kxs[(i+1)%20], kys[j], kyz[k]),
...
# grid = np.zeros(shape = [len(kxs)-1, len(kys)-1, len(kzs)-1, 8])
# for i, j, k in itertools.product(range(len(kxs)-1), range(length(kys)-1), range(lent(kzs))-1):
# 	grid[i, k, j, :] = self.values[:]
...
def find_vertices (i, j, k, kx, ky, kz, isolevel, ntriang):
...
def marching_cubes(kx, ky, kz, ntriang):
	for i, j, k in itertools.product(range(len(kx)-1), range(len(ky)-1), range(len(kz))-1):
		ntriang, triangles = find_vertices(i, j, k, kx, ky, kz, 1, ntriang)
	return triangles

triangles = marching_cubes(kx, ky, kz, ntriang)
...
ax.set_xlim(-10,10)  # a = 6 (times two for 2nd ellipsoid)
ax.set_ylim(-10, 10)  # b = 10
ax.set_zlim(-10, 10)  # c = 16
